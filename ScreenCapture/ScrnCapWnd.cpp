	...
	switch(m_emAction) {
		...
		case ACTION_TEXT: {
			...
			if(!m_bInputText) {
				...
				endPoint.x = m_ptStart.x + RC_TEXTWND_W;
				endPoint.y = m_ptStart.y + RC_TEXTWND_H;
				...
				if((m_rcSel.PtInRectX(m_ptStart) && m_rcSel.PtInRectX(judgePoint) /*&& m_rcTxtSel.PtOutRectX(m_ptStart)*/)) {
					...
					if(m_rcTxtSel.bottomX > m_rcSel.bottomX) {
						...
						m_rcTxtSel.topX = m_rcTxtSel.bottomX - RC_TEXTWND_H;
					}
...
LRESULT CScrnCapWnd::OnKeyDown(WPARAM wParam, LPARAM lParam)
{
	...
	else if(m_emAction > ACTION_CHOOSING) {
		switch(wParam) {
			default:
				break;
			case VK_RETURN: {
...
LRESULT CScrnCapWnd::OnRectangle(WPARAM wParam, LPARAM lParam)
{
	...
	//CString msg = bShow ? "OnRectangle True" : "OnRectangle False";
	//MsgOut(msg);
	CreateColorWnd(bShow);
...
BOOL CScrnCapWnd::SaveAsPic(HBITMAP hImage, const CString& pFileName, const CString& strFormat)
{
	...
	{
		CLSID picClsid;
		CScrnCapWnd::GetEncoderClsid(strFormat.AllocSysString(), &picClsid);
		Bitmap *pBitmap = Bitmap::FromHBITMAP(hImage, NULL);
		bRes = Gdiplus::Ok == pBitmap->Save(pFileName.AllocSysString(), &picClsid);
	}
